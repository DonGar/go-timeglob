...
func (suite *MySuite) TestTimeGlobParseGood(c *check.C) {
	globs := []string{
...
func (suite *MySuite) TestTimeGlobParseBad(c *check.C) {
	globs := []string{
		"",
		"Known Bad Glob",
...
func (suite *MySuite) TestTimeGlobParseVerify(c *check.C) {
	matchesExpected(c, "2015/12/25 19:37 UTC", &TimeGlob{
	...
	matchesExpected(c, "2015,2016,/11,11,12/25,22,25 19,8:22,37 UTC", &TimeGlob{
		[]int{2015, 2016}, []int{11, 12}, []int{22, 25},
		[]int{8, 19}, []int{22, 37},
		time.UTC,
	})
...
func (suite *MySuite) TestTimeGlobParseWildcardEquivalence(c *check.C) {
