...
func (suite *MySuite) TestDateNoNormalize(c *check.C) {
	...
	result := tg.dateNoNormalize(2015, 12, 25, 1, 15)
	...
	// Out of bounds.
	result = tg.dateNoNormalize(2015, 13, 25, 1, 12)
	c.Check(result, check.Equals, UNKNOWN)

	result = tg.dateNoNormalize(2015, 12, 32, 1, 12)
	c.Check(result, check.Equals, UNKNOWN)

	result = tg.dateNoNormalize(2015, 12, 25, 24, 12)
	c.Check(result, check.Equals, UNKNOWN)

	result = tg.dateNoNormalize(2015, 12, 25, 1, 62)
	c.Check(result, check.Equals, UNKNOWN)

	// Leap Days
	result = tg.dateNoNormalize(2015, 2, 29, 0, 0)
	c.Check(result, check.Equals, UNKNOWN)

	result = tg.dateNoNormalize(2016, 2, 29, 0, 0)
